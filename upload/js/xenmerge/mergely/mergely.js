...
$.extend(Mgly.mergely.prototype, 
{
	...
	bind: function() {
		...
		if (this.settings.lhs_cmsettings !== undefined) lhs_cmsettings = jQuery.extend(lhs_cmsettings, this.settings.lhs_cmsettings);
		...
		if (this.settings.rhs_cmsettings !== undefined) rhs_cmsettings = jQuery.extend(rhs_cmsettings, this.settings.rhs_cmsettings);
	...
	_markup_changes: function (editor_name1, editor_name2, changes) {
		...
		for (var i in changes) {
			...
			if (!this.cmsettings.readOnly) {
				...
				$(button).get(0).onclick = (function (change) {
					return function () {
						var scroller = [tthis.editor[editor_name1].getScrollerElement(), tthis.editor[editor_name2].getScrollerElement()];
						var scroll_pos = [{x: scroller[0].scrollLeft, y: scroller[0].scrollTop}, {x: scroller[1].scrollLeft, y: scroller[1].scrollTop}];
						...
						$(scroller[0]).scrollLeft(scroll_pos[0].x).scrollTop(scroll_pos[0].y);
						$(scroller[1]).scrollLeft(scroll_pos[1].x).scrollTop(scroll_pos[1].y);
						return false;
					}
				})(change);
